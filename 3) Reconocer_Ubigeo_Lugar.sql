CREATE or replace PROCEDURE Reconocer_Ubigeo_Lugar()
LANGUAGE plpgsql AS
$$
DECLARE
   --_some_id varchar(20);_var_total int; _var_contador int;
   _var_contadorOferta int; _var_totalOferta int; _var_idoferta int ; _var_lugar varchar(100); _var_idubigeo varchar(6); _var_departamento varchar(100);_var_provinciaUbigeo varchar(100); _var_provinciaBusqueda int;  _var_distrito  varchar(100);  _var_contadorGlobal int; _var_totalGlobal int ; _var_totalprovincias int; _var_totaldistritoes int ; 
...
select id_oferta, lugar  from oferta where lugar is not null;
